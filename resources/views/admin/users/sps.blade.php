  ...
  <div class="content-wrapper">
    <div class="content-header row">
      <div class="content-header-left col-md-6 col-xs-12 mb-1">
        <h2 class="content-header-title">All registered SPs</h2>
      </div>
    ...
    <div class="col-xs-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">All registered Sps</h4>
                        ...
                        <tbody>
                          @if($sps->count()>0)
                                ...
                                <tr>
                                  <td>{{$sps->id}}</td>
                                  <td>{{$sps->name}}</td>
                                  <td>{{$sps->fname}} {{$sps->lname}}</td>
                                  <td>{{$sps->job_title}}</td>
                                  <td>{{$sps->email}}</td>
                                  <td>{{$sps->address}}</td>
                                  <td>
                                        <a href="{{ route('admin.sps.delete', ['id' => $sps->id])}}" class="btn btn-xs btn-danger">Delete</a>
                                  </td>
                                </tr>
                                ...
                                @else
                                  <tr>
                                    <th colspan="5" class="text-center">No Sps.</th>
                                  </tr>
                                @endif
                        </tbody>
                    </table>
