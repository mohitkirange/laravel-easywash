  ...
  <div class="content-wrapper">
    <div class="content-header row">
      <div class="content-header-left col-md-6 col-xs-12 mb-1">
        <h2 class="content-header-title">All registered users</h2>
      </div>
                        ...
                        <tbody>
                          @if($users->count()>0)
                                ...
                                <tr>
                                  ...
                                  <td>{{$users->email}}</td>
                                  <td>{{$users->phone}}</td>
                                  <td>{{$users->address}} {{$users->city}}<br>{{$users->state}}.{{$users->zipcode}}.</td>
                                  <td>
                                    @if( $users->verified == "0")
                                    <span class="bg-danger text-highlight white">Unverified</span>
                                    @else
                                    <span class="bg-success text-highlight white">verified</span>
                                    @endif


                                  </td>
                                  <td>
                                        <a href="{{ route('admin.users.delete', ['id' => $users->id])}}" class="btn btn-xs btn-danger">Delete</a>
                                  </td>
                                </tr>
